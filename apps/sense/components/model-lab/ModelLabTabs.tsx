...
export default function ModelLabTabs({ state, update, pinResult, removePin, clearPins }: Props) {
    ...
    useEffect(() => {
        ...
        apiClient.evaluatorGetModels()
            .then(res => update({ modelCatalogue: res.models }))
            .catch(() => {
                // If backend not yet deployed, show fallback catalogue
    ...
    return (
        <section className="max-w-7xl mx-auto px-4 pb-24">
            ...
            <AnimatePresence mode="wait">
                ...
                >
                    {state.activeTab === 0 && (
                        <UploadScanTab state={state} update={update} />
                    )}
                    {state.activeTab === 1 && (
                        <DocumentViewTab state={state} update={update} />
                    )}
                    {state.activeTab === 2 && (
                        <MetricsTab state={state} update={update} pinResult={pinResult} />
                    )}
                    {state.activeTab === 3 && (
                        <FailuresTab state={state} update={update} />
                    )}
                    {state.activeTab === 4 && (
                        <CompareTab
                            state={state}
...
// Fallback shown before backend responds (matches EVALUATOR_MODEL_CATALOGUE in api.py)
const FALLBACK_CATALOGUE: EvaluatorModel[] = [
    ...
    { key: 'gliner',       label: 'GLiNER Small',      hf_id: 'urchade/gliner_small-v2.1',                          type: 'GLiNER',      params: 'small', f1_benchmark: 0.850, lazy: false, description: 'Zero-shot GLiNER small.' },
    { key: 'deberta',      label: 'DeBERTa PII',       hf_id: 'lakshyakh93/deberta-large-finetuned-pii',            type: 'NER',         params: '86M',   f1_benchmark: 0.920, lazy: false, description: 'Kaggle-winning DeBERTa V3.' },
    { key: 'pasteproof',   label: 'Pasteproof v2',     hf_id: 'joneauxedgar/pasteproof-pii-detector-v2',           type: 'NER',         params: '149M',  f1_benchmark: 0.970, lazy: true,  description: 'ModernBERT 149M.' },
    ...
    { key: 'nvidia_gliner',label: 'NVIDIA GLiNER',     hf_id: 'nvidia/gliner-PII-0.1',                             type: 'GLiNER',      params: '570M',  f1_benchmark: 0.870, lazy: true,  description: 'NVIDIA GLiNER 37-label.' },
