...
export default function SlackScanTab({ modelCatalogue, onStepChange }: Props) {
  ...
  const inputCls = `w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-rose-500 focus:ring-2 outline-none text-slate-900 text-sm`;
        ...
        {step === 'RESULTS' && (
          <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col flex-1 min-h-0">
            <div className="flex items-center justify-between gap-3 bg-white border-b border-slate-200 px-6 py-3 shadow-sm shrink-0">
                ...
                <h2 className="text-sm font-bold text-slate-800 flex-1">Slack Channel: {creds.channel_id}</h2>
