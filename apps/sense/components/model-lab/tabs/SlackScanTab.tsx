...
import { AlertCircle, CheckCircle2, Loader2, ArrowLeft, MessageSquare } from 'lucide-react';
import { apiClient, EvaluatorModel, AnalysisResponse, OutOfCreditsError } from '@/lib/apiClient';
...
export default function SlackScanTab({ modelCatalogue, onStepChange }: Props) {
  ...
  const [creds, setCreds] = useState({ token: '', channel_id: '' });
  const [isScanning, setIsScanning] = useState(false);
  const [scanResult, setScanResult] = useState<AnalysisResponse | null>(null);

  const inputCls = `w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-rose-500 focus:ring-2 outline-none text-slate-900 dark:text-white text-sm`;

  const handleScan = async () => {
    if (!creds.token || !creds.channel_id) return setError('Bot Token and Channel ID are required.');
    ...
    try {
      const res = await apiClient.scanConnector('slack', creds, token || '');
      setScanResult(res);
      changeStep('RESULTS');
    } catch (e: any) { setError(e.message || 'Failed to scan Slack.'); } 
      ...
      <AnimatePresence mode="wait">
        {step === 'AUTH' && (
          <motion.div key="auth" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-4xl mx-auto space-y-6 w-full p-6">
            <Card>
              <h3 className="text-lg font-semibold mb-5 flex items-center gap-2"><span className="flex items-center justify-center w-6 h-6 rounded-full bg-rose-100 text-rose-700 text-sm font-bold">1</span> <MessageSquare className="w-5 h-5 text-rose-500" /> Slack Connection</h3>
              <div className="space-y-4">
                <div><label className="block text-sm font-medium mb-1.5">Bot OAuth Token (xoxb-...)</label><input type="password" value={creds.token} onChange={e => setCreds(p => ({ ...p, token: e.target.value }))} className={inputCls} placeholder="xoxb-123456..." /></div>
                <div><label className="block text-sm font-medium mb-1.5">Channel ID</label><input type="text" value={creds.channel_id} onChange={e => setCreds(p => ({ ...p, channel_id: e.target.value }))} className={inputCls} placeholder="C1234567890" /></div>
                
                <button onClick={handleScan} disabled={isScanning} className={`w-full py-3 bg-rose-600 hover:bg-rose-700 text-white rounded-xl font-medium disabled:opacity-50 flex justify-center items-center gap-2`}>
                  {isScanning ? <Loader2 className="w-5 h-5 animate-spin" /> : null}
                  {isScanning ? 'Scanning Channel...' : 'Connect & Scan Channel'}
                </button>
              </div>
            </Card>
          </motion.div>
        ...
        {step === 'RESULTS' && (
          <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col flex-1 min-h-0">
            <div className="flex items-center justify-between gap-3 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-3 shadow-sm shrink-0">
                ...
                <h2 className="text-sm font-bold text-slate-800 dark:text-slate-200 flex-1">Slack Channel: {creds.channel_id}</h2>
            ...
            <div className="grid grid-cols-3 divide-x divide-slate-200 dark:divide-slate-800 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0">
              <DashboardStatCard label="Messages Scanned" value={scanResult?.rows_scanned || 0} />
