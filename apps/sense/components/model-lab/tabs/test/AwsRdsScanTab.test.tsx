...
import AwsRdsScanTab from '../../AwsRdsScanTab';
...
vi.mock('@/lib/apiClient', async (importOriginal) => {
  ...
  return {
    ...actual,
    apiClient: {
      listAwsRdsTables: vi.fn().mockResolvedValue({ tables: mockTables }),
...
beforeEach(() => {
  const mockTables = Array.from({ length: 25 }, (_, i) => `table_${i + 1}`);
  vi.mocked(apiClient.listAwsRdsTables).mockResolvedValue({ tables: mockTables });
...
describe('AwsRdsScanTab', () => {
  const setupAndClickScan = async (numTablesToSelect: number) => {
    const { container, unmount } = render(<AwsRdsScanTab modelCatalogue={[]} />);
    // Select Postgres
    fireEvent.click(screen.getByRole('button', { name: /🐘 PostgreSQL/i }));
    
    // Auth inputs (using querySelector or placeholder logic)
    // AwRdsScanTab has multiple inputs without great labels for testing, let's just rely on order or placeholders.
    // Host, Port, Database, User, Password
    const inputs = screen.getAllByRole('textbox'); // Host, Port, Database, User
    fireEvent.change(inputs[0], { target: { value: 'localhost' } }); // Host
    fireEvent.change(inputs[1], { target: { value: '5432' } }); // Port
    fireEvent.change(inputs[2], { target: { value: 'db' } }); // Database
    fireEvent.change(inputs[3], { target: { value: 'user' } }); // User
  ...
  it('Table list renders correctly and selection works', async () => {
    const { unmount } = render(<AwsRdsScanTab modelCatalogue={[]} />);
    
    fireEvent.click(screen.getByRole('button', { name: /🐘 PostgreSQL/i }));
    
    const inputs = screen.getAllByRole('textbox');
    fireEvent.change(inputs[0], { target: { value: 'localhost' } });
    fireEvent.change(inputs[1], { target: { value: '5432' } });
    fireEvent.change(inputs[2], { target: { value: 'db' } });
    fireEvent.change(inputs[3], { target: { value: 'user' } });
    ...
    expect(tableDiv).toHaveClass('border-indigo-500'); // Selected class for RDS
    unmount();
  });
