...
function catalogEntryToDriveItem(entry: FileCatalogEntry): DriveItem {
    ...
    return {
        id: entry.file_id, name: entry.file_name, mimeType: entry.connector_type || 'mongodb', path: entry.file_name,
...
export default function MongodbScanTab({ modelCatalogue, onStepChange }: Props) {
  ...
  const [creds, setCreds] = useState<DatabaseCredentials>({
    host: '', port: '27017', database: '', user: '', password: '',
  });
  ...
  const accentBg = 'bg-green-100 text-green-700';
  const accentStep = 'bg-green-100 text-green-700';
  ...
  const fetchCatalog = async () => {
    try {
        const res = await apiClient.getDbCatalog(token || '', 'mongodb');
  ...
  const handleConnect = async () => {
    ...
    try {
      const res = await apiClient.listMongodbCollections(creds);
  ...
  const handleScan = async (mode: 'full' | 'metadata' = 'full') => {
    ...
    for (const t of tablesToScan) {
      try {
        ...
        if (mode === 'metadata') {
          const catalogRes = await apiClient.scanMetadata({ ...creds, connector_type: 'mongodb', table: t }, token || '');
      ...
      <AnimatePresence mode="wait">
        {step === 'AUTH' && (
          <motion.div key="auth" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-4xl mx-auto space-y-6 w-full p-6">
            <Card>
              <h3 className="text-lg font-semibold mb-5 flex items-center gap-2"><StepBadge n={1} accentStep={accentStep} /> 🍃 MongoDB Connection</h3>
              <div className="space-y-4">
                ...
                <div><label className="block text-sm font-medium mb-1.5">User</label><input type="text" value={creds.user} onChange={e => setCreds(p => ({ ...p, user: e.target.value }))} className={inputCls} placeholder="admin" /></div>
                ...
                <button onClick={handleConnect} disabled={isConnecting} className={`w-full py-3 ${accentBtn} text-white rounded-xl font-medium disabled:opacity-50`}>
                  {isConnecting ? 'Connecting...' : 'Connect & List Collections'}
                </button>
              </div>
            </Card>
          </motion.div>
        ...
        {step === 'BROWSE' && (
          <motion.div key="browse" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col flex-1 min-h-0 bg-white dark:bg-slate-900">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800">
              <span className="font-bold dark:text-white">Select Collections ({selectedTableIds.size}/{tables.length})</span>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-2">
              {tables.map(t => (
                <div key={t} onClick={() => setSelectedTableIds(p => { const n = new Set(p); n.has(t) ? n.delete(t) : n.add(t); return n; })}
                  className={`p-3 rounded-xl border cursor-pointer dark:text-slate-200 ${selectedTableIds.has(t) ? 'border-green-500 bg-green-50 dark:bg-green-500/10' : 'border-slate-200 dark:border-slate-700'}`}>
                  {t}
                </div>
              ))}
            </div>
            {selectedTableIds.size > 0 && (
              <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 flex gap-4">
                ...
                <button onClick={() => handleScan('full')} className="flex-1 py-2 bg-green-600 text-white rounded-lg text-sm font-bold">Full Deep Scan</button>
              </div>
            )}
          </motion.div>
        )}
                ...
                <div className="p-6">
                    <div className="flex flex-col items-center justify-center py-12 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm">
                        ...
                        <div className="flex items-center gap-2">
                            <p className="text-slate-500 dark:text-slate-400 text-sm">Downloading and scanning collections in-memory…</p>
