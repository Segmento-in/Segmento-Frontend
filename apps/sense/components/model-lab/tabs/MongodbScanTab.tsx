        ...
        {step === 'BROWSE' && (
          <motion.div key="browse" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col flex-1 min-h-0 bg-white">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <span className="font-bold">Select Collections ({selectedTableIds.size}/{tables.length})</span>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-2">
              {tables.map(t => (
                <div key={t} onClick={() => setSelectedTableIds(p => { const n = new Set(p); n.has(t) ? n.delete(t) : n.add(t); return n; })}
                  className={`p-3 rounded-xl border cursor-pointer ${selectedTableIds.has(t) ? 'border-green-500 bg-green-50' : 'border-slate-200'}`}>
                  {t}
                </div>
              ))}
            </div>
