...
export default function DriveScanTab({ modelCatalogue, onStepChange }: Props) {
    ...
    const stats = useMemo(() => {
        ...
        const piiFromCatalog = fileItems.filter(i => {
            ...
            return cat?.classification === 'SENSITIVE' || i.appProperties?.segmento_pii_detected === 'true' || i.appProperties?.segmento_pii_detected === true;
        }).length;
        const cleanFromCatalog = fileItems.filter(i => {
            if (scannedIds.has(i.id)) return false;
            const cat = catalogData.find(c => c.file_id === i.id);
            return cat?.classification === 'NON-SENSITIVE' || i.appProperties?.segmento_pii_detected === 'false' || i.appProperties?.segmento_pii_detected === false;
        }).length;
        ...
        const newFiles = fileItems.filter(i => {
            ...
            if (cat?.classification === 'SENSITIVE' || cat?.classification === 'NON-SENSITIVE') return false;
