...
function catalogEntryToDriveItem(entry: FileCatalogEntry): DriveItem {
    ...
    return {
        id: entry.file_id, name: entry.file_name, mimeType: entry.connector_type || 'mariadb', path: entry.file_name,
...
export default function MariadbScanTab({ modelCatalogue, onStepChange }: Props) {
  ...
  const [creds, setCreds] = useState<DatabaseCredentials>({
  ...
  const accentRing = 'focus:ring-sky-500';
  const accentBtn = 'bg-sky-600 hover:bg-sky-700';
  const accentBg = 'bg-sky-100 text-sky-600';
  const accentStep = 'bg-sky-100 text-sky-600';
  ...
  const fetchCatalog = async () => {
    try {
        const res = await apiClient.getDbCatalog(token || '', 'mariadb');
  ...
  const handleConnect = async () => {
    ...
    try {
      const res = await apiClient.listMariadbTables(creds);
  ...
  const handleScan = async (mode: 'full' | 'metadata' = 'full') => {
    ...
    for (const t of tablesToScan) {
      try {
        ...
        if (mode === 'metadata') {
          const catalogRes = await apiClient.scanMetadata({ ...creds, connector_type: 'mariadb', table: t }, token || '');
          ...
          res = await apiClient.scanConnector('mariadb', { ...creds, table: t }, token || '');
        }
      ...
      <AnimatePresence mode="wait">
        {step === 'AUTH' && (
          <motion.div key="auth" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-4xl mx-auto space-y-6 w-full p-6">
            <Card>
              <h3 className="text-lg font-semibold mb-5 flex items-center gap-2"><StepBadge n={1} accentStep={accentStep} /> 🦭 MariaDB Connection</h3>
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-3">
                  <div className="col-span-2">
                    <label className="block text-sm font-medium mb-1.5">Host</label>
                    <input type="text" value={creds.host} onChange={e => setCreds(p => ({ ...p, host: e.target.value }))} className={inputCls} placeholder="localhost" />
                  </div>
                  <div>
                ...
                <div><label className="block text-sm font-medium mb-1.5">Database</label><input type="text" value={creds.database} onChange={e => setCreds(p => ({ ...p, database: e.target.value }))} className={inputCls} placeholder="my_database" /></div>
                <div><label className="block text-sm font-medium mb-1.5">User</label><input type="text" value={creds.user} onChange={e => setCreds(p => ({ ...p, user: e.target.value }))} className={inputCls} placeholder="root" /></div>
                <div><label className="block text-sm font-medium mb-1.5">Password</label><input type="password" value={creds.password} onChange={e => setCreds(p => ({ ...p, password: e.target.value }))} className={inputCls} placeholder="••••••••" /></div>
          ...
          <motion.div key="browse" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col flex-1 min-h-0 bg-white">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-2">
              {tables.map(t => (
                <div key={t} onClick={() => setSelectedTableIds(p => { const n = new Set(p); n.has(t) ? n.delete(t) : n.add(t); return n; })}
                  className={`p-3 rounded-xl border cursor-pointer ${selectedTableIds.has(t) ? 'border-sky-500 bg-sky-50' : 'border-slate-200'}`}>
                  {t}
                </div>
              ))}
            </div>
            {selectedTableIds.size > 0 && (
              <div className="p-4 border-t bg-slate-50 flex gap-4">
                ...
                <button onClick={() => handleScan('full')} className="flex-1 py-2 bg-sky-600 text-white rounded-lg text-sm font-bold">Full Deep Scan</button>
              </div>
            )}
          </motion.div>
        )}
        ...
        {step === 'RESULTS' && (
          <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col flex-1 min-h-0">
            ...
            <ConnectorPreviewUI items={catalogItems} selectedIds={new Set()} onToggleSelection={() => {}} scanningIds={scanningTableIds} scanResults={[]} onOpenFile={() => {}} connectorType="mariadb" catalogData={catalogData} lastSession={lastSession} filterMode="all" searchQuery="" className="flex-1 min-h-0" mode="database" isMetadataScan={lastScanMode === 'metadata'} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
