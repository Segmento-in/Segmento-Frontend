            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-2">
              {tables.map(t => (
                <div key={t} onClick={() => setSelectedTableIds(p => { const n = new Set(p); n.has(t) ? n.delete(t) : n.add(t); return n; })}
                  className={`p-3 rounded-xl border cursor-pointer dark:text-slate-200 ${selectedTableIds.has(t) ? 'border-sky-500 bg-sky-50 dark:bg-sky-500/10' : 'border-slate-200 dark:border-slate-700'}`}>
                  {t}
                </div>
              ))}
            </div>
