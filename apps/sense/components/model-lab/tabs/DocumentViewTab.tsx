...
const RESULT_COLORS = {
    ...
    DET: { bg: 'rgba(99,102,241,0.25)', border: '#6366F1', label: '' },
};
...
export default function DocumentViewTab({ state, update }: Props) {
    ...
    const highlightedHtml = useMemo(() => {
        ...
        if (hasGT) {
            modelData.comparison.TP.forEach(e =>
                spans.push({
                    start: e.start,
                    end: e.end,
                    result: 'TP',
                    label: e.canonical || e.label,
                    score: e.score,
                })
            );

            modelData.comparison.FP.forEach(e =>
                spans.push({
                    start: e.start,
                    end: e.end,
                    result: 'FP',
            ...
            modelData.comparison.FN.forEach(e =>
                spans.push({
                    start: e.start,
                    end: e.end,
                    result: 'FN',
            ...
            modelData.predictions.forEach(e =>
                spans.push({
                    start: e.start,
                    end: e.end,
                    result: 'DET',
        ...
        for (const s of clean) {
            ...
            html += `<mark style="background:${c.bg};border-bottom:2px solid ${c.border};border-radius:3px;padding:1px 3px;cursor:help;" title="${escapeHtml(
                tooltip
    ...
    return (
        <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-wrap items-center gap-4">
                ...
                {/* Doc page navigation */}
                {state.parseResult &&
                    state.parseResult.doc_count > 1 && (
                        <div className="flex items-center gap-2 ml-auto">
                            <button
                                disabled={state.docIndex <= 0}
                                onClick={() =>
                                    update({
                                        docIndex: state.docIndex - 1,
                                    })
                                }
                                className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 disabled:opacity-30 hover:border-slate-300 dark:hover:border-slate-600 transition-colors"
                            >
                                <ChevronLeft size={14} />
                            </button>

                            <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">
                                Doc {(state.docIndex ?? 0) + 1} /{' '}
                                {state.parseResult.doc_count}
                            </span>
            ...
            {/* Legend */}
            {hasGT && (
                <div className="flex flex-wrap gap-3 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
            ...
            {/* Document content */}
            <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm p-6 overflow-x-auto">
                {text ? (
                    <p
                        className="text-sm text-slate-700 dark:text-slate-300 leading-7 whitespace-pre-wrap font-mono"
                    ...
                    {[
                        {
                            ...
                            value: modelData.comparison.TP.length,
                            color:
                                'text-emerald-500 dark:text-emerald-400',
                        },
                        {
                            label: 'False Positives',
                            value: modelData.comparison.FP.length,
                            color:
                                'text-orange-500 dark:text-orange-400',
                        },
                        {
                            label: 'False Negatives',
                            value: modelData.comparison.FN.length,
                            color: 'text-red-500 dark:text-red-400',
                        },
                    ].map(s => (
                        <div
                            key={s.label}
                            className="flex flex-col items-center gap-1 py-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm"
                        >
                            <span
...
function Empty({ message }: { message: string }) {
    return (
        <div className="flex flex-col items-center justify-center gap-4 py-24 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
            <AlertCircle
                size={32}
