...
import { apiClient, EvaluatorModel, AnalysisResponse, PIICount, DatabaseCredentials, FileCatalogEntry, DriveItem } from '@/lib/apiClient';
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'RESULTS' && (
          ...
          >
            ...
            {/* ── Results table ─ flex-1 overflow-y-auto */}
            <div className="flex-1 overflow-y-auto bg-white">
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-slate-50 border-b border-slate-200 z-10">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-500 w-56">
                      Table Name
                    </th>
                    <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-500">
                      Classification
                ...
                <tbody className="divide-y divide-slate-100">
                  {scanEntries
                    ...
                    .filter(e =>
                      resultSearch
                        ? e.tableName.toLowerCase().includes(resultSearch.toLowerCase())
                        : true,
                    )
                    .map(entry => (
                      <React.Fragment key={entry.tableName}>
                        <tr className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-mono font-semibold text-slate-800">
                                {creds.database}.
                                <span className="font-bold">{entry.tableName}</span>
                              </span>
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3">
                            ...
                            {entry.status === 'scanned' && entry.result && (
                              entry.result.total_pii_found > 0 ? (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-red-300 bg-red-50 text-red-700 text-xs font-black uppercase tracking-widest">
                                  <AlertCircle className="w-3 h-3" />
                                  PII Positive
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-emerald-300 bg-emerald-50 text-emerald-700 text-xs font-black uppercase tracking-widest">
                            ...
                            {entry.status === 'error' && (
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-red-50 text-red-500 text-xs rounded-full border border-red-200">
                                <AlertCircle className="w-3 h-3" />
                                Error
                              </span>
                            )}
                          ...
                          <td className="px-4 py-3">
                            {entry.status === 'scanned' && entry.result ? (
                              <span className={`font-bold ${entry.result.total_pii_found > 0
                                  ? 'text-red-600'
                                  : 'text-emerald-600'
                                }`}>
                                {entry.result.total_pii_found > 0
                                  ? entry.result.total_pii_found
                                  : 'Clean'}
                              </span>
                            ) : (
                          ...
                          {/* Column count */}
                          <td className="px-4 py-3">
                            {entry.status === 'scanned' && entry.result?.schema ? (
                              <span className="text-slate-500 text-xs">
                                {entry.result.schema.length} cols
                              </span>
                            ) : (
                              <span className="text-slate-300">—</span>
                            )}
                          ...
                          <td className="px-4 py-3">
                            {entry.status === 'scanning' ? (
                              ...
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-50 text-emerald-600 text-xs rounded-full border border-emerald-200">
                                <CheckCircle2 className="w-3 h-3" />
                                Scanned
                              </span>
                            ) : entry.status === 'error' ? (
                        ...
                        {/* Expandable PII breakdown row */}
                        {entry.status === 'scanned' && entry.result &&
                          entry.result.pii_counts &&
                          entry.result.pii_counts.length > 0 && (
                        ...
                        {entry.status === 'error' && entry.error && (
                          <tr className="bg-red-50/20">
                            <td colSpan={5} className="px-8 py-2 text-xs text-red-500">
                              {entry.error}
                            </td>
                          </tr>
                        )}
