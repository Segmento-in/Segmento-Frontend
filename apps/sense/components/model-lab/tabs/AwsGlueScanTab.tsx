...
function catalogEntryToDriveItem(entry: FileCatalogEntry): DriveItem {
    ...
    return {
        id: entry.file_id, name: entry.file_name, mimeType: entry.connector_type || 'glue', path: entry.file_name,
...
export default function AwsGlueScanTab({ modelCatalogue, onStepChange }: Props) {
  ...
  const inputCls = `w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-orange-500 focus:ring-2 outline-none text-slate-900 dark:text-white text-sm`;
